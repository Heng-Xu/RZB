#!/usr/bin/env python3
"""校验终稿正文、Word、PDF、关键数值及推荐矩阵，并生成内部追溯表。"""
...
FORBIDDEN = [
    ...
    "证据等级", "证据链", "候选包", "artifact", "manifest", "pipeline", "fallback",
...
def run_check(root: Path, markdown: Path, docx: Path, pdf: Path, output_dir: Path) -> dict:
    ...
    expected_equations = [f"3-{i}" for i in range(1, 7)] + [f"4-{i}" for i in range(1, 5)]
    ...
    checks = [
        ...
        ("第五章", "表5-1", "QX-00001无上限年化规划成本", formal.loc["QX-00001", "PATH_OPT_CLR_UNBOUNDED_cumulative_eac"], "| QX-00001 | 无上限 | 2.3597466 | 50 | 19 | 1.9 | 4.085 | 188.1800 |", "188.1800"),
        ("第五章", "表5-1", "QX-00001控制值2.0年化规划成本", formal.loc["QX-00001", "PATH_OPT_CLR_LE_2_cumulative_eac"], "| QX-00001 | 控制值2.0 | 2.2679276 | 0 | 138 | 13.8 | 29.670 | 481.9566 |", "481.9566"),
        ...
        ("第五章", "表5-1", "QX-00005无上限年化规划成本", formal.loc["QX-00005", "PATH_OPT_CLR_UNBOUNDED_cumulative_eac"], "| QX-00005 | 无上限 | 2.1715720 | 50 | 465 | 46.5 | 99.975 | 1745.5283 |", "1745.5283"),
        ("第五章", "表5-1", "QX-00005控制值2.0年化规划成本", formal.loc["QX-00005", "PATH_OPT_CLR_LE_2_cumulative_eac"], "| QX-00005 | 控制值2.0 | 2.1192953 | 0 | 962 | 96.2 | 206.830 | 3359.0522 |", "3359.0522"),
    ...
    for chapter, item, metric, official, context, report_value in checks:
        passed = context in source and report_value in source[source.find(context):source.find(context) + len(context) + 160]
        if not passed:
    ...
    if len(backtest) != 8 or not backtest["explains_formal_optimization"].all():
        issues.append("推荐矩阵未合理解释全部片区")
    if int(backtest["matches_formal_optimization"].sum()) != 7:
        issues.append("矩阵直接一致片区数量应为7，QX-00007应保留为缺失数据例外")
    if backtest["artificial_upper_bound"].any():
        issues.append("推荐矩阵存在人为上限")
    ...
    for forbidden_range in ("0.90～1.25", "1.161～1.523", "大于1.25且不高于1.60"):
        if forbidden_range in source:
            issues.append(f"推荐矩阵仍含单样本外推范围：{forbidden_range}")
    for contradiction in ("技术可行性和控制值是否改变规划方案", "新片区应用时，应使用同期数据"):
        if contradiction in source:
            issues.append(f"正文存在矩阵应用定义冲突：{contradiction}")
    ...
    if math_count != 10:
        issues.append(f"Word原生公式数量应为10，实际为{math_count}")
    ...
    if "\\frac" in document_xml or "\\Delta" in document_xml:
    ...
    for unit in ("年化规划成本/万元·年⁻¹", "网络容量支撑裕度", "局部最大反向潮流比例"):
        if unit not in source:
            issues.append(f"术语或单位缺失：{unit}")
    figure_script = (root / "实验/研究/scripts/build_final_report_figures.py").read_text(encoding="utf-8")
    for old_term in ("累计年化成本 / 万元", "线路容量最小余度", "局部最大反向功率比"):
...
def main() -> int:
    ...
    result = run_check(args.root.resolve(), args.markdown.resolve(), args.docx.resolve(), args.pdf.resolve(), args.output_dir.resolve())
