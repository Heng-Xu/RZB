...
| `env MPLCONFIGDIR=/tmp/mplcfg_xuzhou conda run -n xuzhou110kv_clr bash scripts/runtests.sh` | 进行中 | 包含两个真实 v3 端到端子测试；第一份临时 `manifest.json` 已于 15:11:39 生成，启动后仍等待最终测试汇总，暂不虚构退出码 |
...
| `git ls-remote --heads origin`（提升网络权限） | 0 | `48acbd4a93ad3ba16c203ad0e335360bc33b0717 refs/heads/main` |
...
1. 等待并登记全量回归最终退出码；若耗时继续增长，以进程/临时 manifest 证据记录，不把历史 207 项绿灯冒充为本次复跑结果。
